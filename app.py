...
def get_stock_base_data():
    ...
    df['industry'] = df['industry'].fillna('女性/其他')
            ...
            with st.spinner(f"正在分析 {len(res)} 檔股票的即時技術指標..."):
                ...
                try:
                    ...
                    if not hist_data.empty:
                        # 改用安全字典儲存，避免 DataFrame 的欄位不對齊造成的 KeyError
                        dd_dict = {}
                        chg_dict = {}
                        ...
                        if isinstance(hist_data.columns, pd.MultiIndex):
                            for code in valid_codes:
                                try:
                                    # 模糊尋找包含該代號與指標的欄位
                                    close_col = [col for col in hist_data.columns if 'Close' in col and code in str(col)]
                                    high_col = [col for col in hist_data.columns if 'High' in col and code in str(col)]
                                    
                                    if close_col and high_col:
                                        series_close = hist_data[close_col[0]].ffill().dropna()
                                        series_high = hist_data[high_col[0]].ffill().dropna()
                                        
                                        if len(series_close) >= 2 and len(series_high) > 0:
                                            max_h = series_high.max()
                                            last_c = series_close.iloc[-1]
                                            prev_c = series_close.iloc[-2]
                                            
                                            dd_dict[code] = round(((max_h - last_c) / max_h * 100), 2) if max_h > 0 else 0.0
                                            激增_ratio = round(((last_c - prev_c) / prev_c * 100), 2) if prev_c > 0 else 0.0
                                            chg_dict[code] = 激增_ratio
                                except Exception:
                                    pass
                        # 處理單層欄位架構 (例如只剩一檔股票時)
                        else:
                            for code in valid_codes:
                                try:
                                    # 尋找含有 Close 或 High 且包含代號的欄位
                                    close_cols = [c for c in hist_data.columns if 'Close' in str(c)]
                                    high_cols = [c for c in hist_data.columns if 'High' in str(c)]
                                    
                                    if close_cols and high_cols:
                                        series_close = hist_data[close_cols[0]].ffill().dropna()
                                        series_high = hist_data[high_cols[0]].ffill().dropna()
                                        ...
                                        if len(series_close) >= 2 and len(series_high) > 0:
                                            ...
                                            dd_dict[code] = round(((max_h - last_c) / max_h * 100), 2) if max_h > 0 else 0.0
                                            chg_dict[code] = round(((last_c - prev_c) / prev_c * 100), 2) if prev_c > 0 else 0.0
                                except Exception:
                                    pass
                                    
                        # 將解析出來的安全數據對齊回主 DataFrame
